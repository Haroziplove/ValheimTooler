...
namespace ValheimAdminToolLauncher.Utils
{
    public class MonoInjector
    {
        ...
        public bool Inject()
        {
            string assemblyPath = Path.Combine(_launcherPath, "ValheimAdminTool.dll");

            if (!File.Exists(assemblyPath))
            {
                return DoError("Cannot found ValheimAdminTool.dll!");
            }
            ...
            using (injector)
            {
                ...
                try
                {
                    remoteAssembly = injector.Inject(assembly, "ValheimAdminTool", "Loader", "Init");
                }
